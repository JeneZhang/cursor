...
同一个 X 会话有两条出口：agent 走 `xdotool` + `x11grab`，人走 noVNC。两者**并行生效**，
人可以在 agent 操作的同时看画面，也可以直接动鼠标接管。

这一点是"图形会话"相比"无头浏览器"最实际的好处：出问题的时候人能直接看到现场。
...
设计上的理由清楚（§3.2）：整条链路没有任何一处知道界面稳定了没有，所以只能等。
执行上还做了两点优化：

- 只在"需要沉降"的动作后面等——鼠标类、`key`、`scroll`，以及**含换行的** `type`
  （不含换行的纯输入不触发）。
- **一批动作只等一次**，不是每个动作都等。所以 `[click, wait, click, screenshot]`
  总共只付一次 2000 ms。

但取值是保守估计，不是测量结果。实测画面 100~250 ms 就稳定，2 秒里约 93% 是空转
